# 🌸 Prem Foundation Nashik - Official Website  

[🌐 Visit Website](https://premfoundation.co.in/)  

---

## 🏢 About Prem Foundation  
Prem Foundation is a **Nashik-based NGO established in 2012**, dedicated to transforming lives through **compassion, community service, and elderly care**.  

### ❤️ Premangan Old Age Home  
- A loving home for senior citizens with professional care  

### 🌱 Community Development Programs  
- Environmental initiatives, education support, and welfare services  

### 🤝 Social Welfare Services  
- Supporting underserved communities through various programs  

---

## 🌟 Website Features  
- Modern & Responsive Design  
- Fully adaptable layout (mobile, tablet, desktop)  
- Optimized for **fast loading**  
- Accessibility-focused design  
- **100% SEO Optimized** with structured metadata  

---

## 📑 Key Pages  
- **Home (`index.html`)** → Overview of our mission and impact  
- **Premangan (`premangan.html`)** → Details about elderly care facility  
- **Donate (`donate.html`)** → Secure donation portal with multiple payment options  
- **Thank You (`thankyou.html`)** → Auto-generated invoice & receipt after donation  
- **About Us (`about.html`)** → Our story, team, and annual reports  
- **Projects (`projects.html`)** → All initiatives and programs  
- **Volunteer (`volunteer.html`)** → Opportunities to get involved  
- **Contact (`contact.html`)** → Ways to reach us  

---

## ⚙️ Technical Features  
- Razorpay **payment integration** for secure online donations  
- **Google Sheets integration** → Automatic storage of donor & form data  
- **Google Forms** → Volunteer and contact data collection  
- **SEO Optimization** → 100% optimized with meta tags, sitemaps, and schema  
- Social media integration  
- Google Analytics tracking  
- SSL secured connections  

---

## 📬 Contact Information  

### 📍 Prem Foundation Office  
4th Floor, Sitaram Chambers, Peth Naka, Panchavati  
Nashik, Maharashtra, India - 422003  
📞 +91 9970559373  
📧 premfoundation2012@gmail.com  

### 🏠 Premangan Old Age Home  
Sanit Bunglow, Near Bhujbal Farm Road, Behind Deccan Petrol Pump  
Sunderban Colony, Lekha Nagar, Nashik, Maharashtra, India - 422009  

---

🌐 **Website:** [https://premfoundation.co.in/](https://premfoundation.co.in/)  
